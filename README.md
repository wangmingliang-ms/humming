...
`~/.humming/settings.json`，并把 setup 会话登记为 Bridge lifecycle 通知目标。整个过程不会在
...
新 Agent。`update` 完成构建后会把重启交给独立 lifecycle coordinator，因此 Bridge 自身退出后仍能
继续完成拉起和就绪检查。

...
**一键把飞书 / Lark 变成本地 Coding Agent 客户端，然后只用自然语言管理它。**

Humming 是飞书 / Lark 与
[ACP](https://agentcommunicationprotocol.dev/core-concepts/architecture) Agent 之间的本地网关服务。
...
| ✨ 一键配置                                                              | 💬 自然语言控制                                                    | 🔄 自更新与重启                                                             |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| 运行 `humming setup`，按链接确认，即可自动创建 Bot、保存凭据并登记通知。 | 直接说“切到 Copilot，用 Agent mode 和 GPT-5.6”，无需记忆管理命令。 | 直接说“更新 Humming 并重启”，Agent 会完成构建并把重启交给独立 coordinator。 |
...
- **一个聊天，多种 Agent**：每个 topic 都可以独立选择 Agent、Model、Mode、Permission 和 ACP
  Config。
- **自然语言控制**：直接说“切到 Copilot 的 Agent mode，用 GPT-5.6”，Humming Agent 会查询
  capabilities、校验并应用完整 profile。
- **一键配置 Bot**：运行一次 `humming setup`，按终端链接确认即可自动创建飞书 / Lark Bot、
  保存凭据并登记生命周期通知，无需手工逐项配置开放平台。
- **理解本地 repo**：chat 绑定本地仓库，topic 持有独立 Agent Session。
- **原生聊天体验**：思考、工具调用、Markdown 结果和权限请求都通过飞书 / Lark 互动卡片呈现。
- **会话可恢复**：Agent 支持时，Gateway 或机器重启后可以继续原有会话。
...
**Gateway** 是一个长期运行的本地进程。一个 chat 持有 repo binding，每个 chat topic 则持有独立的
...
例如，`claude` preset 使用维护中的 Claude Agent SDK ACP adapter，并沿用本机 Claude
Code 的认证。
...
这是推荐的一键配置流程。打开终端打印的 setup 链接，在飞书中登录，选择或创建群聊，并确认创建
Bot；Humming 会自动完成适合 Agent 使用的应用注册，把返回的凭据保存到
`~/.humming/settings.json`，并把 setup 会话登记为 Gateway lifecycle 通知目标。整个过程不会在
终端打印 App Secret，也不需要手工逐项添加权限、事件和回调。
...
Humming 会把当前 chat/topic scope 和完整操作指引提供给 Agent。除了正常编码任务，你可以直接说：

```text
把当前 topic 切到 Copilot，使用 Agent mode 和 GPT-5.6，权限改成 alwaysAsk，
然后继续处理刚才的任务。
```

```text
先查一下 Claude 支持哪些 model 和 mode，不要切换。
```

```text
把这个 chat 绑定到 /home/me/work/my-project。
```

```text
更新 Humming 到最新版并重启 Gateway。
```

Agent 会先查询实际 capabilities，再通过 Humming CLI 完成操作。Agent、Model、Mode、Permission、
Config 与附带任务可以作为一次 profile handoff 原子应用：只有完整 profile 生效后，任务才会发送给
新 Agent。`update` 完成构建后会把重启交给独立 lifecycle coordinator，因此 Gateway 自身退出后仍能
继续完成拉起和就绪检查。

### Slash commands

普通消息会交给当前 Agent。需要确定性快捷操作时，也可以使用下面这些由 Humming 直接处理的 slash
command。
...
| Preset         | 接入方式                                                         |
| -------------- | ---------------------------------------------------------------- |
| `claude`       | 维护中的 Claude Agent SDK ACP adapter，沿用本机 Claude Code 认证 |
| `claude-agent` | `claude` 的兼容别名                                              |
...
只有前台 `gateway run` 接受 raw command。如果需要稳定地在后台运行，请在 `settings.json` 中添加
自定义 preset。
...
顶层进程命令是快捷方式：`humming start` 与 `humming gateway start` 完全等价。使用
...
`start` 会保存启动配置，把日志写入 `~/.humming/gateway.log`，并在 Humming home 下记录受管
PID。Linux 上可用时会使用 systemd user service，其他环境使用 detached process。

restart 和 stop 是协调式生命周期操作：Gateway 会先停止接收新任务、等待正在处理的 turn、保存
...
| `gateway.log`              | 后台 Gateway 日志                                      |
| `gateway.pid`              | 受管进程标识                                          |
| `gateway.launch.json`      | 已保存的 Gateway 启动配置                              |
...
managed installation 可以自行更新。既可以在终端执行：

```bash
humming update
```

也可以直接在聊天中说“更新 Humming 到最新版并重启”。Agent 会调用同一个更新流程。

update 会 fetch `origin`，把 managed checkout 硬同步到 `origin/$HUMMING_REF`（默认 `main`），
安装依赖、构建、刷新全局链接，并使用已保存的启动配置重启正在运行的 Gateway。managed checkout
中的本地修改会被覆盖。

...
describe("conversation card lifecycle is unconditional", () => {
  it("ignores the removed feature setting", () => {
    ...
    try {
      ...
      const config = readConfigFile(settingsPath) as unknown as Record<string, unknown>;
  ...
  it("rejects the removed cards-v2 management command", () => {
    expect(() => parseArgs(["cards-v2", "enable"])).toThrow(/unexpected positional argument/);
  });
});

# SPEC: Unified `~/.humming/` home dir + natural-language binding
...
1. **Scattered state.** humming currently spreads its files across three
   places — confirmed on this machine:
   - `~/.config/humming/config.json` (credentials + runtime + agents)
   - `~/.local/share/humming/sessions.json` + `bindings.json`
   - log wherever the launcher points (`/tmp/humming.log` in our runs)
...
- Consolidate everything under a single home dir **`~/.humming/`**, created
  on startup if missing. It holds config, logs, session/binding state — the
  one place humming manages.
- Merge config into a single **`~/.humming/settings.json`** (credentials +
  runtime + agents + bindings all in one file).
- Natural-language binding **without MCP**: the agent edits `settings.json`
  using its own file tools; humming watches the file and hot-reloads. No new
...
### PHASE 1 — Unify to `~/.humming/` (pure infra refactor, no behaviour change)
...
- Layout inside:
  ...
  ~/.humming/
    ...
    humming.log       # default log target (see 1.4)
...
```jsonc
{
  ...
  "runtime": {
    ...
    "unboundCwd": "~/.humming", // phase 2: reception area for unbound chats
  },
...
- `FileSessionStore` — point at `~/.humming/sessions.json` (constructor
...
- CLI default: write pino output to `~/.humming/humming.log` when no TTY /
...
On startup, if `~/.humming/settings.json` does NOT exist:

1. If old `~/.config/humming/config.json` exists → read it, and read old
   `~/.local/share/humming/bindings.json` if present → compose a fresh
   `settings.json` (config fields + `bindings` block) and write it.
2. Copy `~/.local/share/humming/sessions.json` → `~/.humming/sessions.json`
   if the new one is absent.
3. Leave the old files in place (non-destructive); log a one-line
   "migrated to ~/.humming" notice.
...
- `HUMMING_APP_ID` / `HUMMING_APP_SECRET` / `HUMMING_PERMISSION_MODE` env
  still override.
...
Depends on Phase 1's `~/.humming/` + settings.json.
...
- Today an unbound chat (no binding, no default) replies "please /bind" and
  ...
  `~/.humming`), spawn the agent there so the user can converse immediately.
...
- Fix: when spawning, inject `HUMMING_CHAT_ID=<chatId>` into the agent env,
  ...
  in ~/.humming/settings.json. Your chat id is $HUMMING_CHAT_ID."
- So the agent writes the correct `bindings[chatId]` entry, not a guess.

#### 2.3 Hot-reload (NEW — does not exist today)

- Pitfall: humming reads config only at startup; nothing watches files.
...
- Concurrency pitfall: humming ALSO writes settings.json (e.g. its own
  `/bind` command, migration). Guard with atomic writes (write temp file +
  rename) on humming's side, and tolerate transient parse failures on the
...
- Unbound chat → can chat with the agent in `~/.humming`.
- User says "bind me to copilot-rewrite" → agent edits settings.json →
  humming hot-reloads → next message runs in `~/workspace/copilot-rewrite`.
...
1. Log rotation for `~/.humming/humming.log` — probably out of scope v1.
